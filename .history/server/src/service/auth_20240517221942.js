...
export const loginService = ({phone, password}) => new Promise(async(resolve, reject) => {
    try{
       ...
       const isCorrectPassword = response && bcrypt.compareSync(password, response.password)
        const token = isCorrectPassword && jwt.sign({id: response[0].id, phone: response[0].phone}, process.env.SECRET_KEY, {expiresIn: '1d'})
        resolve({
            err: token ? 0 : 2,
            msg: token ? 'Đăng nhập thành công' : response ? 'Mật khẩu sai' : 'Không tìm thấy tài khoản',
            token: token || null
        })
