...
const Navbar = ({setnav}) => {
    ...
    const activeItemNV = useSelector((state: any) => state.reducer.main1Slice.activeItem);
    useEffect(() => {
        // Đặt item hoạt động từ localStorage nếu có sẵn
        const storedActiveItem = Number(localStorage.getItem('activeItem')) || 1;
        dispatch(setActiveItem(storedActiveItem));
    }, [dispatch]);

    useEffect(() => {
        // Lưu item hoạt động vào localStorage mỗi khi nó thay đổi
        localStorage.setItem('activeItem', activeItemNV.toString());
    }, [activeItemNV]);
