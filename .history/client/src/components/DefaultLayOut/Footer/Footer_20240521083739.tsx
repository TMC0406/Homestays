        ...
        <div className=" grid grid-cols-4 border-b-[1px] border-[black]">
            {
              dataFooter.map((item)=>{
                return (
                  <div key={item.title} className="p-[20px]">
                    <div className="">
                    ...
                    <div className="">
                      {item.listChildTitle.map((item2)=>{
                          return ( 
                            <div key={item2.ChildTitle} className="">
          ...
          <div className="p-[10px] border-r-[1px] border-gray-300">
          ...
          </div>
            {
              dataFooter2.map((item)=>{
                return(
                  <div  key={item.title}  className="p-[10px]">
                    <h1 className="text-[1rem] font-[700]  text-black hover:text-blue-400">{item.title}</h1>
                    {
                      item.listChildTitle.map((item2)=>{
                        return(
                          <p key={item2.ChildTitle}  className="text-[1rem] font-[500]  text-black hover:text-blue-400">{item2.ChildTitle}</p>
                        )
                      })
                    }
                  </div>
                    ...
                    <div className=" grid grid-cols-4">
                      {
                        item.listImg.map((item2)=>{
                          return(
                            <img key={item2.imgLink}  src={item2.imgSrc} alt="" className='w-[70%] h-[70%]  p=[10px] m-[10px]' />
                          )
                        })
                      }
                    </div>
