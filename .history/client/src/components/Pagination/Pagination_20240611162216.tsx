      ...
      {/* <ItemDetail/> */}
        ...
        <div className="box-btn">
          ...
          {number.map((it) => (
            ...
            //   {it}
            // </button>
            <Button text={it} onClick={() => handleChange(it)} className={`m-[5px] p-[5px] border-[2px] ${currentPage === it ? "bg-red-700 text-white" : ""}`} icon={undefined}/>
          ))}
          {/* <button  className="m-[5px] p-[5px] border-[2px]"  onClick={handleNext}>Next</button> */}
          <Button text={'Trang sau'} onClick={handleNext} className={'m-[5px] p-[5px] border-[2px]'} icon={undefined}/>
        </div>
      </div>
    </div>
  );
