    ...
    return (
        <div className='w-full flex mb-[10px] py-[15px]  border-t-[1px] border-solid border-[#E13427]'>
            <div className='w-[40%] flex flex-wrap gap-[2px] items-center relative'>
               {images.length > 0 &&  images.filter((i, index) =>  indexs.some(i =>  i === index))?.map(i => {
                return(
                    <img src={i} alt="" className='w-[140px] h-[120px] object-cover' />
                )
               })}
                <span className='bg-[#00000080] absolute bottom-[8px] left-[5px] text-[#fff] text-center w-[40px] h-[20px] rounded-[3px] text-[12px] px-[5px]'>{`${images?.length}`} ảnh</span>
                <span
            ...
            <div className='w-[60%] ml-[15px]'>
                ...
                <div className='mb-[10px] w-full h-[50px] text-ellipsis overflow-hidden'>
