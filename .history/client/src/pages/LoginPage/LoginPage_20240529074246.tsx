...
const handleSubmit = async () => {
  if(isRegister){
    try {
      ...
      dispatch({ actions: actionTypes.REGISTER_FAIL, data: error.msg });
    }
  }else{
    try {
      const response = await apiLogin(payload);
       dispatch({ actions: actionTypes.LOGIN_SUCCESS, data: response.data })
       navigate('/');
       alert("Đăng nhập thành công!")
      ;
    } catch (error) {
      dispatch({ actions: actionTypes.LOGIN_FAIL, data: error.msg });
    }
  }
